  ...
  switch ((z.append(fragment), (s = z.select("#flight-computer")), scope)) {
    case "wind":
      $(".wind-link").toggleClass("active"),
        ...
        (plotArea = s.select("#plot-area")),
        (dot = s.select("#plot-dot")),
        (xStart = compass.getBBox().cx),
        (yStart = compass.getBBox().cy),
        (yShift = 0),
        (angle = 0),
        (lastAngle = 0),
        (startAngle = 0),
        (minDragMove = 0),
        plotArea.drag(dragPlotArea, startPlotArea, stopPlotArea),
        trueIndex.drag(dragTrueIndex, startTrueIndex, stopTrueIndex),
        compass.drag(dragCompass, startCompass, stopCompass);
      break;
    case "sliderule":
      setupSlideRule();
      break;
...
function getSVG(q, path, files) {
  switch (q) {
    case "wind":
      return path + files.wind;
    case "sliderule":
      return path + files.slideRule;
    default:
      return path + files.slideRule;
  }
}
function setupSlideRule() {
  $(".sliderule-link").toggleClass("active"),
    (tasBase = s.select("#tas-base")),
    (tasDial = s.select("#tas-dial")),
    (xStart = tasBase.getBBox().cx),
    (yStart = tasBase.getBBox().cy),
    (angle = 0),
    (lastAngle = 0),
    (startAngle = 0),
    tasDial.drag(dragDial, startDial, stopDial);
}
function getLeftMargin() {
  var width = { outer: $("#outer").width(), pan: $pan.width() },
    margin = (width.outer - width.pan) / 2;
  return margin;
}
  ...
  switch (scope) {
    case "wind":
      (xStart = compass.getBBox().cx),
        ...
        (startAngle = 0),
        (minDragMove = 0),
        (resetItems = [plotGroup, trueIndex, plotArea, compass, dot]),
...
!(function (a) {
  ...
  (function (a, b) {
    ...
    var c = (function (b) {
      ...
      d = (function () {
        ...
        return (
          (d.getElementByPoint = function (a, b) {
            var c = this,
  ...
  (function (window, document, $) {
    function getSVGFallback() {
      ...
      if (null !== fallback) {
        var svgFile = this.src.split("/").pop(),
          src = this.src.replace(svgFile, fallback);
        return src;
      }
      return this.src;
    }
